...
hit_modes_aux = [((hit+10)//20)*20 for hit in hit_hist]
miss_modes_aux = [((miss+10)//20)*20 for miss in miss_hist]
...
if hit_modes[-1] < miss_modes[0]:
    if (hit_modes[0] - 10 <= 0):
        ...
        low_t.append(hit_modes[0] - 10)
    high_t.append((hit_modes[-1]+miss_modes[0])//2)
elif miss_modes[-1] < hit_modes[0]:
    print("[Abnormal machine behavior]",
            "Highest miss latency is lower than lowest hit latency")
    print("hit modes: ", hit_modes)
    print("miss modes: ", miss_modes)
    low_t.append((hit_modes[0]+miss_modes[-1])//2)
    high_t.append(hit_modes[-1]+10)
    confidence = 50
else:
    miss_id = 0
    hit_id = 0
        ...
        if hit_modes[hit_id] < miss_modes[miss_id]:
            ...
            for i in range(hit_id, len(hit_modes)):
                if hit_modes[i] > miss_modes[miss_id]:
                    ...
                    hit_end = i
            low_t.append(hit_modes[hit_id]-10)
...
hit_modes_aux = [((hit+10)//20)*20 for hit in hit_hist[3*ITERATIONS//4:ITERATIONS]]
hit_modes = []
for i in range(10):
    if len(hit_modes_aux) < ITERATIONS/4000 \
        or len(stats.mode(hit_modes_aux)[1]) < 1 \
...
for i in range(3*ITERATIONS//4,ITERATIONS):
    step_width += 1
    if ((hit_hist[i]+10)//20)*20 <= hit_modes[0]:
...
if SHOW_PLOT:
    ...
    fig, ax = plt.subplots()
    plt.plot(x,hit_hist,color='r',label="reload hit")
    plt.plot(x,miss_hist,color="yellow",label="reload miss")
    axes = plt.gca()
